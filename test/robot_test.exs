defmodule ToyRobot.RobotTest do
  ...
  describe "when the robot is facing north, and it has moved forward a space" do
    setup do
      {:ok, %{robot: %Robot{facing: :north, north: 1}}}
    end
  ...
  describe "when the robot is facing east, and it has moved north once" do
    setup do
      {:ok, %{robot: %Robot{facing: :east, north: 1}}}
    end
  ...
  describe "when the robot is fascing south, and it has moved east once" do
    setup do
      {:ok, %{robot: %Robot{facing: :south, east: 1}}}
    end

    test "moves south onve space", %{robot: robot} do
      robot = robot |> Robot.move
      assert robot.north == -1
      assert robot.east == 1
      assert robot.facing == :south
